function SIRmatrix = modelSIR_P4(S0, I0, R0, T0, T, h, b0, A, omega, gamma)
    ...
    for i = 1:size(days,2)-1
        K(1,1) = dSdt(susepList(i),infectList(i),B(i));
        K(2,1) = dIdt(susepList(i),infectList(i),B(i));
        K(3,1) = dRdt(infectList(i));

        K(1,2) = dSdt(susepList(i) +K(1,1) * h/2, infectList(i) + K(2,1) * h/2,B(i));
        K(2,2) = dIdt(susepList(i) +K(1,1) * h/2, infectList(i) + K(2,1) * h/2,B(i));
        K(3,2) = dRdt(infectList(i) + K(2,1) * h/2);

        K(1,3) = dSdt(susepList(i) +K(1,2) * h/2, infectList(i) + K(2,2) * h/2,B(i));
        K(2,3) = dIdt(susepList(i) +K(1,2) * h/2, infectList(i) + K(2,2) * h/2,B(i));
        K(3,3) = dRdt(infectList(i) + K(2,2) * h/2);

        K(1,4) = dSdt(susepList(i) +K(1,3) * h, infectList(i) + K(2,3) * h,B(i));
        K(2,4) = dIdt(susepList(i) +K(1,3) * h, infectList(i) + K(2,3) * h,B(i));
